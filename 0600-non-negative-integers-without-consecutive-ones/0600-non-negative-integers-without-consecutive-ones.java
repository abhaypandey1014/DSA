class Solution {
    public int solve(String s,int idx,int dp[][][][],int tight,int start,int prev){
        ...
        int limit = (tight==1)?s.charAt(idx)-'0':1; /// max limit can be 1
        int ans = 0;
        for(int i = 0;i<=limit;i++){
            int newTight = (tight==1 && i==limit)?1:0;
            if(start==0 && i==0) ans += solve(s,idx+1,dp,newTight,0,0);
            else{
                //consecutive 1 hai to skip kro wrna aage forward kro
    ...
    public int help(String s){
        int dp[][][][] = new int[s.length()+1][2][2][2];//idx,tight,start,prev
    ...
    public int findIntegers(int n) {
        //for binary conversion because we are dealing with binary form
